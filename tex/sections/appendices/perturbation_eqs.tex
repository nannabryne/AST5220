...
We let $' \equiv \dv{}{x}$ in the equations below. Additionally, we introduce $\ckH = \ckH(k, x)\equiv \frac{ck}{\Hp(x)}$ as a shorthand notation.
...
Let $\Psi\ped{init}\equiv\Psi(k, x\ped{init})$ and $k\eta(x\ped{init}) \ll 1$. Further, we use that $\Psi\ped{init}=-\sfrac{2}{3}$ and the following scalar quantities at $x=x\ped{init}$:
\begin{subequations}\label{app:pert:eq:initial_conditions}
\begin{align}
    % \Psi(k, x\ped{init})     &= \Psi\ped{init} =-\frac{2}{3}     \label{app:pert:eq:init_Psi}\\
