...
When $\Gamma_1 \ll \Hp\eu^{-x}$, the interaction is irrelevant and the r.h.s.~of~\cref{mil2:theo:eq:Boltzmann} is practically zero and $n_1$ goes as $\eu^{-3x}$, meaning that the number density of $(1)$ is constant in a comoving volume. If $(1)$ is massive, we say that it \textit{freezes out} at this point, and expect to find the same fractional abundance today. That is, of course, when ignoring the epoch of reionisation, which would complicate the evolution of the quantities in this section quite severely.

% \footnote{\textcolor{blue}{Comment about reionisation?}}
