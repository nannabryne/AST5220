...
\subsubsection{Optical depth and visibility}\label[sec]{mil2:imp:sec:tau_gt}
    We computed $\tau(x)$ by solving the simple ODE in~\cref{mil2:theo:eq:ode_tau_of_x} starting from $x=0$ where $\tau(0)=0$ and integrating backwards, using $8\cross10^5$ steps along $x\in[0, -12]$ in the RK4 algorithm.
