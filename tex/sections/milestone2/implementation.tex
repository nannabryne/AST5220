...
\subsubsection{Sound horizon}\label[sec]{mil2:imp:sec:rs}
    The computation of $r_s(x)$ was a straight-forward numerical integration of~\cref{mil2:theo:eq:ode_rs_of_x} for $x\in[-12, 0]$, again using RK4 with $8\cross10^5$ steps. Having $x\ped{init}=-12$ ensures radiation domination (see \cref{mil1:res:fig:density_params}), so the initial condition is valid.