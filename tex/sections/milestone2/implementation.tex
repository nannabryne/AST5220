...
We implemented a class in \verb|C++| that assumes a background, an object of the class from \cref{mil1:sec:imp}, and a primordial helium abundance $Y_P$. We wrote code that (1) calculated $X_e(x)$ (and $n_e(x)$) from the \textcolor{blue}{Saha and Peebles equations} and (2) computed $\tau(x)$ as well as $\gti(x)$, $\dv*[]{\gti(x)}{x}$ and $\dv*[2]{\gti(x)}{x}$ for a given array of $x$-values. %We chose the interval $x\in [-14, 2]$.
%subsections??
