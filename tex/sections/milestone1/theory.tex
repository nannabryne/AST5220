...
\begin{equation}\label{mil1:theo:eq:FRW}
    \begin{split}
        d s^2 &= -c^2\diff t^2 + a^2(t) \kroneckerdelta{ij} \diff x^i \diff x^j && |\, \diff \eta \equiv c\diff t\, a^{-1}(t) \\%\frac{\diff t}{a(t)}  \\
                &= a^2(t) \left( -\diff \eta^2 + \kroneckerdelta{ij} \diff x^i \diff x^j \right).
    \end{split}
\end{equation}
