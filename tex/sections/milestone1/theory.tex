...
\subsubsection{Cosmic expansion}\label[sec]{mil1:theo:sec:approximations}
    ...
    \begin{subequations}\label{mil1:theo:eq:time_initial_conditions}
        \begin{align}
            \eta\ped{init} &= \int_{-\infty}^{x\ped{init}} \frac{c\eu^{x}}{H_0 \sqrt{\OmgR}} = \frac{c}{\Hp(x\ped{init})} \label{mil1:theo:eq:eta_init} \\
            t\ped{init} &=  \int_{-\infty}^{x\ped{init}} \frac{\eu^{x}\eu^{x}}{H_0 \sqrt{\OmgR}} =\frac{\eu^{x\ped{init}}}{2 \Hp(x\ped{init})} \label{mil1:theo:eq:t_init} 
        \end{align}
    \end{subequations}
    Choosing such initial conditions, it is important to make sure $x\ped{init} \ll x\ped{eq}$ so that the approximation on $\Hp(x)$ is viable. 
...
\subsubsection{Distance measures}\label[sec]{mil1:theo:sec:distances}
    ...
    \end{equation}
    of which the right-hand side is the comoving distance $\chi= \eta_0-\eta$. We evaluate the left integral in~\cref{mil1:theo:eq:integral_for_r} and find
