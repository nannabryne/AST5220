...
\end{figure}
The contributions to the CMB spectrum are calculated as follows, were $\mathrm{[comp]}\in \{\,\mathrm{SW},\,\mathrm{ISW},\,\mathrm{Doppler},\,\mathrm{pol} \,\}$ are the terms in the source function ($\St(x,k) \supset (\mathrm{[comp]})$, see \cref{mil4:theo:eq:source_function_four_terms}) given by~\cref{mil4:theo:eq:source_terms}:
\begin{equation}
\begin{split}
    &\mathcal{D}(\ell)^\mathrm{[comp]} = \frac{\ell (\ell +1 ) (\TCMB\unit{[\text{\textmu} K]})^2}{2\pi} C(\ell)^\mathrm{[comp]}\,; \\
...
\begin{figure}[!ht]
    ...
    \caption{The total matter power spectrum (today) $P\ped{m0}(k)$ as function of wavenumber $k$. Both axes are logarithmic. The equality scale $k\ped{eq}$ is demonstrated by a vertical dotted line.} 
