...
\subsubsection{Effective temperature perturbation}\label[sec]{mil4:theo:sec:eff_temp_pert}
    ...
    The equation for this quantity describes a driven harmonic oscillator, and going back to real space and ignoring the driving force, we obtain an equation for $\delta_\gamma(t, \vec{x})$ describing waves moving at the speed of sound $c_s=1/\sqrt{3}$. Further, it can be shown that $\left[\Theta_0 + \Psi\right](x, k) \propto \cos(ks(x))$ if we keep neglecting the driving force. A rough mapping $k\to \ell/\eta_0$ lets us predict the position of the peaks in $C(\ell)$,
    \begin{equation}
        \ell\ped{pk} \sim n \cross \frac{\eta_0}{r_s}\, ; \quad n\in \mathbb{N}.
    \end{equation}
    Likewise, the throughs will be positioned at
    \begin{equation}
        \ell\ped{pk} \sim \left(n+\frac{1}{2}\right) \cross \frac{\eta_0}{r_s}\, ; \quad n\in \mathbb{N}.
    \end{equation}
