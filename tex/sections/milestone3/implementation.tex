...
The following recipe~\ref{item_one} --~\ref{item_six} was used for each of the 100 logarithmically spaced values $k\in [k\ped{min},\, k\ped{max}]$, where $k\ped{min} = 5\cross10^{-5}\unit{Mpc}^{-1}$ and $k\ped{max}3\cross 10^{-1}\unit{Mpc}^{-1}$.
...
We used an array of $5\cross 10^4+1$ values of $x\in [x\ped{init}, 0]$ with $x\ped{init}=-20$. In the full regime, we used $\ell\ped{max}=7$.
