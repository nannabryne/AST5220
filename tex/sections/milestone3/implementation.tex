...
\subsubsection{Perturbations}\label[sec]{mil3:imp:sec:main_work}
    The following recipe \ref{item_one} -- \ref{item_six} was used for each of the 100 logarithmically spaced values $k \in [5\cross10^{-5}, \,3\cross 10^{-1}]\unit{Mpc}$. We used an array of $1\cross 10^4+1$ values of $x\in [x\ped{init}, 0]$ with $x\ped{init}=-20$.

    \begin{enumerate}[wide,labelwidth=!,labelindent=0pt,label=(\roman*)]
        \item\label{item_one} 
        For the initial time $x\ped{init}$, set the initial conditions according to~\cref{app:pert:sec:initial_conditions} (\cref{app:pert:eq:initial_conditions} and~\eqref{app:pert:eq:dTheta0dx} to~\eqref{app:pert:eq:dTheta1dx}). 
        ...
        Use the result from the tight coupling regime at $x=x\ped{tc,end}(k)$ to set the initial conditions for the full system. The higher multipoles are set according to~\cref{app:pert:eq:init_Theta2} to~\eqref{app:pert:eq:dThetaelldx}.
