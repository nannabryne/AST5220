...
\subsubsection{Tight coupling}
    ...
    The higher moments of the temperature fluctuations are very small in this regime ($\abs{\Theta_{\ell\geq 2}}\ll 1$), and we can use the semi-aWinalytical recursive relations in~\cref{app:pert:eq:initial_conditions_Theta2ell}. However, we will not bother to compute others than $\Theta_2$, as this is the only one our set of equations rely on. As we will see, there is a much cleverer way of obtaining the higher order multipoles, if our overall goal is to study today's values.
