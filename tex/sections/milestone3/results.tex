...
From this point (Sects.~\ref{sec:mil3} \& \ref{sec:mil4}), the results were obtained \emph{after} reproducing the background with $N\ped{eff}=0$. This caused some changes in numerical values for e.g.~matter-radiation equality, but did not change the important results and takeaways from the previous work (Sects.~\ref{sec:mil1} \& \ref{sec:mil2}). 
...
\begin{figure}[!ht]
    ...
    \caption{The graphs show the photon quadrupole $\Theta_2(x, k)$ as function of logarithmic scale factor $x$ for three different wavenumbers $k$. The shaded grey regions represent the epochs of radiation-matter equality and receombination. Horizon entries are shown as thick, vertical coloured lines.} 
...
\begin{table}[h]
    ...
    \input{tables/milestone3/wavenumbers.tex}
\end{table}
